...
CSSLint.addRule({
  ...
  init: function(parser, reporter) {
    ...
    function property(event) {
      event.value.parts.forEach(function(part) {
        if (
          ...
          reporter.report(
            `Colors should be in hexadecimal notation: ${part} (${part}=${toHex(part.text)})`,
            part.line,
            part.col,
            rule
          );
    ...
    function toHex(colour) {
      if (colour.indexOf('rgb') === 0) return rgbToHex(colour);
      else return colourNameToHex(colour);
    }

    function rgbToHex(colour) {
      return colour.replace(/rgb\((\d+)\s*,\s*(\d+)\s*,\s*(\d+)\)/, function(_, r, g, b) {
